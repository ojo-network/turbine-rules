...
## Market Question
**Will Ethena's USDe stablecoin depeg before December 31, 2025, 11:59 PM UTC?**
...
4. **Data Source Verification**: The depeg is confirmed by at least **3 out of 5** designated price oracle sources

### Primary Data Sources (Price Oracles)

The following sources shall be used for price determination, in order of priority (if available):

1. **CoinGecko API** - USDe/USD pricing data
2. **CoinMarketCap API** - USDe/USD pricing data
3. **Curve Finance** - USDe/USDT, USDe/USDC, and USDe/DAI pool pricing
4. **Uniswap V3** - USDe/USDC and USDe/USDT pool TWAP (Time-Weighted Average Price)
5. **Chainlink Oracle** - USDe/USD price feed

### Timing Specification
- **End Time**: Market closes at December 31, 2025, 23:59:59 UTC
...
- If a depeg occurs on **only one exchange** due to that exchange's technical issues (e.g., deposit/withdrawal suspension, exchange hack not affecting USDe contract), it does NOT count unless 3+ oracle sources confirm the depeg
- If **multiple major exchanges** (3+) simultaneously show depeg due to their own technical issues (not USDe-related), this does NOT count as a legitimate depeg
...
- If 3+ primary data sources are **unavailable** at the time of resolution, voters should:
...
- The deadline of "December 31, 2025, 11:59 PM" means **23:59:59 UTC on December 31, 2025**
- Any depeg event must **start** before this deadline; if it starts at 23:55 UTC and continues into 2026, it still counts
...
### Resolution Timeline
- Resolution should occur within **7 days** after the market end date (by January 7, 2026)
- If disputes arise, allow additional time for evidence gathering and discussion

## Definitions

- **USDe**: Ethena's synthetic dollar stablecoin as deployed on Ethereum mainnet at the official contract address
...
- [ ] At least 3 out of 5 oracle sources confirm the depeg
- [ ] The depeg period began before December 31, 2025, 23:59:59 UTC
