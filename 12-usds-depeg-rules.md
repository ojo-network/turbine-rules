# MakerDAO USDS Depeg Resolution Rules

## Market Question
**Will MakerDAO's USDS stablecoin depeg before February 28, 2026, 11:59 PM UTC?**
...
1. **Price Threshold**: USDS trades at a price **outside the range of $0.95 to $1.05** (i.e., greater than 5% deviation from $1.00 peg)
...
**Aggregators:**
1. **CoinGecko API** - USDS/USD pricing data
2. **CoinMarketCap API** - USDS/USD pricing data

**Decentralized Exchanges:**
3. **Curve Finance** - USDS/USDC, USDS/USDT, and 3pool pricing
4. **Uniswap V3** - USDS/USDC and USDS/ETH pool TWAP (Time-Weighted Average Price)
5. **Manifest DEX** - USDS pricing data

**Centralized Exchanges:**
6. **Binance** - USDS/USD pricing data
7. **MEXC** - USDS/USD pricing data
8. **Bullish** - USDS/USD pricing data
9. **Hotcoin** - USDS/USD pricing data
10. **Biconomy** - USDS/USD pricing data
11. **CoinUp** - USDS/USD pricing data

**On-Chain Oracles:**
12. **Chainlink Oracle** - USDS/USD price feed

### Timing Specification
- **Market Start**: February 9, 2026, 00:00:00 UTC
- **End Time**: Market closes at February 28, 2026, 23:59:59 UTC
...
- If a depeg is caused by a **confirmed exploit, hack, or smart contract vulnerability** affecting USDS or its primary liquidity pools, it SHALL count as a depeg
...
- If a depeg occurs on **only one exchange** due to that exchange's technical issues (e.g., deposit/withdrawal suspension, exchange hack not affecting USDS contract), it does NOT count unless 2+ oracle sources confirm the depeg
- If **multiple major exchanges** (2+) simultaneously show depeg due to their own technical issues (not USDS-related), this does NOT count as a legitimate depeg

### 3. Liquidity Events
- **Low liquidity conditions**: If total USDS liquidity across all major DEXs falls below $50M, price deviations during this period require additional scrutiny
...
#### USDS Contract Changes
- If MakerDAO **deploys a new USDS contract** and initiates a migration:
  - The old USDS contract price is tracked until migration completes or old token liquidity becomes negligible (<$10M)
  - The new contract is tracked from deployment
  - A depeg in EITHER version counts as a depeg event

#### Upgrades and Pauses
- If the USDS contract or MakerDAO protocol is **paused** by governance or emergency functions, this is considered a depeg event if it lasts 6+ hours and trading cannot occur
- If the contract is **upgraded** and the upgrade causes price disruption, this counts as a depeg if criteria are met

### 5. DSR (USDS Savings Rate) Considerations
- Changes to the **USDS Savings Rate (DSR)** do NOT constitute a depeg unless they cause USDS market price to deviate beyond the threshold
- If sUSDS (staked USDS) depegs but USDS maintains its peg, this does NOT count

### 6. Collateral and Backing Issues
- If MakerDAO's **collateral backing** falls below required levels, but USDS maintains its peg on markets, this does NOT count as a depeg
- If **backing concerns** or liquidation events cause a market depeg that meets all criteria, it DOES count
- If a **black swan event** triggers Emergency Shutdown, evaluate USDS market price during the event

### 7. Regulatory and Legal Events
- If USDS is **delisted** from exchanges due to regulatory action, evaluate remaining exchanges for price
- If MakerDAO/Sky **ceases operations** but USDS contract remains functional, track the market price
...
- If Ethereum is **unreachable** for 6+ hours preventing USDS transfers, this is NOT automatically a depeg unless price data shows depeg

#### Multi-chain Deployments
- If USDS is **bridged to other chains** (e.g., Arbitrum, Optimism, Polygon), only the **Ethereum mainnet** USDS price is used for resolution unless >70% of liquidity migrates to another chain

### 9. MakerDAO Governance Events
- If MakerDAO governance **votes to change USDS's peg target** (e.g., to a different value), the market resolves based on the original $1.00 peg
- If **Emergency Shutdown** is triggered, evaluate USDS market price during and after the event

### 10. Market Closure Scenarios
- If **all major exchanges** simultaneously delist or suspend USDS trading, and no price can be determined for 6+ hours, this SHALL be considered a depeg event
...
- The deadline of "February 28, 2026, 11:59 PM" means **23:59:59 UTC on February 28, 2026**
- Any depeg event must **start** before this deadline; if it starts at 23:55 UTC and continues into March, it still counts

### 13. Ambiguous Situations
If a situation arises that is not clearly covered by these rules, voters should:
1. Prioritize the **spirit of the market** (did USDS meaningfully lose its peg?)
...
### Resolution Timeline
- Resolution should occur within **7 days** after the market end date (by March 7, 2026)
- If disputes arise, allow additional time for evidence gathering and discussion

## Definitions

- **USDS**: MakerDAO's decentralized stablecoin as deployed on Ethereum mainnet at the official contract address (0x6B175474E89094C44Da98b954EesadFd8AD7Ff5F3e)
...
- **Price Oracle**: An API or on-chain data source providing reliable USDS/USD pricing information
...
2. MakerDAO makes fundamental changes to USDS that require rule updates
...
- USDS price was below $0.95 OR above $1.05
